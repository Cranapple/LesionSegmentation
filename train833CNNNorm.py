...
with tf.device(device_name):
	...
	cov2_norm_mean = tf.Variable(tf.ones([depth1]), trainable=False)
	cov3_norm_mean = tf.Variable(tf.ones([depth2]), trainable=False)
	cov4_norm_mean = tf.Variable(tf.ones([depth2]), trainable=False)
	cov5_norm_mean = tf.Variable(tf.ones([depth2]), trainable=False)
	cov6_norm_mean = tf.Variable(tf.ones([depth3]), trainable=False)
	cov7_norm_mean = tf.Variable(tf.ones([depth3]), trainable=False)
	cov8_norm_mean = tf.Variable(tf.ones([depth3]), trainable=False)

	cov1_norm_var = tf.Variable(tf.zeros([depth1]), trainable=False)
	cov2_norm_var = tf.Variable(tf.zeros([depth1]), trainable=False)
	cov3_norm_var = tf.Variable(tf.zeros([depth2]), trainable=False)
	cov4_norm_var = tf.Variable(tf.zeros([depth2]), trainable=False)
	cov5_norm_var = tf.Variable(tf.zeros([depth2]), trainable=False)
	cov6_norm_var = tf.Variable(tf.zeros([depth3]), trainable=False)
	cov7_norm_var = tf.Variable(tf.zeros([depth3]), trainable=False)
	cov8_norm_var = tf.Variable(tf.zeros([depth3]), trainable=False)

	cov1_norm_scale = tf.Variable(tf.ones([depth1]))
	cov2_norm_scale = tf.Variable(tf.ones([depth1]))
	cov3_norm_scale = tf.Variable(tf.ones([depth2]))
	cov4_norm_scale = tf.Variable(tf.ones([depth2]))
	cov5_norm_scale = tf.Variable(tf.ones([depth2]))
	cov6_norm_scale = tf.Variable(tf.ones([depth3]))
	cov7_norm_scale = tf.Variable(tf.ones([depth3]))
	cov8_norm_scale = tf.Variable(tf.ones([depth3]))

	cov1_norm_beta = tf.Variable(tf.zeros([depth1]))
	cov2_norm_beta = tf.Variable(tf.zeros([depth1]))
	cov3_norm_beta = tf.Variable(tf.zeros([depth2]))
	cov4_norm_beta = tf.Variable(tf.zeros([depth2]))
	cov5_norm_beta = tf.Variable(tf.zeros([depth2]))
	cov6_norm_beta = tf.Variable(tf.zeros([depth3]))
	cov7_norm_beta = tf.Variable(tf.zeros([depth3]))
	cov8_norm_beta = tf.Variable(tf.zeros([depth3]))
	...
	def model(data, isTraining=True):
		...
		conv = tf.nn.conv2d(convNorm, cov5_weights, [1, 1, 1, 1], padding='VALID')				#cov5
		hidden = tf.nn.relu(conv + cov5_biases)
		convNorm = batch_norm_wrapper(hidden, cov5_norm_scale, cov5_norm_beta, cov5_norm_mean, cov5_norm_var, is_training=isTraining)
		if(isTraining):
			convNorm = tf.nn.dropout(convNorm, dropoutRate)

		conv = tf.nn.conv2d(convNorm, cov6_weights, [1, 1, 1, 1], padding='VALID')			#cov6
		hidden = tf.nn.relu(conv + cov6_biases)
		convNorm = batch_norm_wrapper(hidden, cov6_norm_scale, cov6_norm_beta, cov6_norm_mean, cov6_norm_var, is_training=isTraining)
		if(isTraining):
			convNorm = tf.nn.dropout(convNorm, dropoutRate)

		conv = tf.nn.conv2d(convNorm, cov7_weights, [1, 1, 1, 1], padding='VALID')			#cov7
		hidden = tf.nn.relu(conv + cov7_biases)
		convNorm = batch_norm_wrapper(hidden, cov7_norm_scale, cov7_norm_beta, cov7_norm_mean, cov7_norm_var, is_training=isTraining)
		if(isTraining):
			convNorm = tf.nn.dropout(convNorm, dropoutRate)

		conv = tf.nn.conv2d(convNorm, cov8_weights, [1, 1, 1, 1], padding='VALID')			#cov8
		hidden = tf.nn.relu(conv + cov8_biases)
		convNorm = batch_norm_wrapper(hidden, cov8_norm_scale, cov8_norm_beta, cov8_norm_mean, cov8_norm_var, is_training=isTraining)
		if(isTraining):
			convNorm = tf.nn.dropout(convNorm, dropoutRate)
	...
	loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=tf.reshape(logits, [-1, 2]), labels=tf.reshape(tf_train_labels, [-1, 2]))
			...
			+ l2Rate*tf.nn.l2_loss(cov5_norm_scale)
			+ l2Rate*tf.nn.l2_loss(cov6_norm_scale)
			+ l2Rate*tf.nn.l2_loss(cov7_norm_scale)
			+ l2Rate*tf.nn.l2_loss(cov8_norm_scale)
			...
			+ l2Rate*tf.nn.l2_loss(cov5_norm_beta)
			+ l2Rate*tf.nn.l2_loss(cov6_norm_beta)
			+ l2Rate*tf.nn.l2_loss(cov7_norm_beta)
			+ l2Rate*tf.nn.l2_loss(cov8_norm_beta)
			, name="loss")
	# Optimizer.
